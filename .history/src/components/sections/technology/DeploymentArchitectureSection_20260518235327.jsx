  ...
  return (
    <SectionShell id="deployment-architecture">
      ...
      <div className="p-6 rounded-xl border border-telemetry-primary/20 bg-telemetry-primary/5 mb-12">
        <h3 className="text-base font-bold text-txt-primary mb-4">Core Properties</h3>
      ...
      {/* Deployment Models */}
      <div className="mb-12">
        <h3 className="text-base font-bold text-txt-primary mb-6 text-center">Deployment Models</h3>
        <div className="grid md:grid-cols-3 gap-4">
          {deploymentModels.map((model, i) => (
            <div key={i} className="p-5 rounded-lg border border-border-subtle bg-bg-primary/50">
              <h4 className="text-sm font-bold text-telemetry-primary mb-2">{model.name}</h4>
              <p className="text-xs text-txt-secondary leading-relaxed mb-3">{model.description}</p>
              
              <div className="space-y-1 mb-3">
                {model.components.map((comp, j) => (
                  <p key={j} className="text-[11px] text-txt-secondary flex items-start gap-1.5">
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        {Object.values(infrastructure).map((item, i) => (
          <div key={i} className="p-5 rounded-lg border border-border-subtle bg-bg-primary/50">
            <h4 className="text-sm font-bold text-txt-primary mb-2">{item.title}</h4>
