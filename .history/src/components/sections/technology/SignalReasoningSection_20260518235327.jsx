  ...
  return (
    <SectionShell id="signal-reasoning">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
        {signalChannels.map((ch, i) => (
          ...
          >
            ...
            <p className="text-xs text-txt-secondary leading-relaxed mb-2">{ch.description}</p>
      ...
      <div className="p-6 rounded-xl border border-border-default bg-bg-primary/60 mb-8">
        <h3 className="text-base font-bold text-txt-primary mb-2">{fusionArchitecture.title}</h3>
        <p className="text-xs text-txt-secondary mb-4">{fusionArchitecture.description}</p>

        <div className="space-y-2">
          {fusionArchitecture.weights.map((w, i) => (
            <div key={i} className="flex items-center gap-3">
              <span className="text-xs font-mono text-telemetry-primary w-10 text-right">{w.weight}</span>
              <div className="flex-1 h-[1px] bg-metallic-800/30" />
              <span className="text-xs text-txt-primary font-semibold">{w.source}</span>
              <span className="text-[11px] text-metallic-500 hidden sm:inline">— {w.role}</span>
            </div>
          ))}
        </div>

        <p className="mt-4 text-xs text-txt-secondary italic">{fusionArchitecture.principle}</p>
      </div>

      {/* Classification requirement */}
      <div className="p-4 rounded-lg border border-telemetry-primary/20 bg-telemetry-primary/5 mb-8">
        <p className="text-sm text-txt-secondary leading-relaxed">{classificationRequirement}</p>
      </div>
