...
export function CognitionRuntimeSection() {
  ...
  return (
    <SectionShell id="cognition-runtime">
        ...
        {runtimeComponents.map((comp, i) => (
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <h4 className="text-base font-bold text-txt-primary">{comp.name}</h4>
            </div>
            <p className="text-xs text-txt-secondary leading-relaxed mb-3">{comp.description}</p>
            ...
            {comp.characteristics && (
              <div className="flex flex-wrap gap-2">
                {comp.characteristics.map((item, j) => (
                  <span key={j} className="inline-block px-2 py-1 text-[11px] text-txt-secondary bg-charcoal-900/80 border border-border-subtle rounded">
                    {item}
                  </span>
                ))}
              </div>
            ...
            {comp.logs && (
              <div className="space-y-1">
                {comp.logs.map((log, j) => (
                  <p key={j} className="text-[11px] text-txt-secondary font-mono">{log}</p>
                ))}
              </div>
            )}
            {comp.interfaces && (
      ...
      <div className="mb-8">
        <h3 className="text-base font-bold text-txt-primary mb-4 text-center">Execution Pipeline</h3>
          ...
          {executionFlow.map((step, i) => (
            <div key={i} className="relative flex items-center gap-2">
              <div className="flex-1 px-3 py-2 bg-bg-primary rounded border border-border-default text-center">
                <span className="text-[10px] text-telemetry-primary/60 block">{i + 1}</span>
                <span className="text-[11px] text-txt-secondary leading-tight">{step}</span>
              </div>
            </div>
          ))}
        </div>
