  ...
  return (
    ...
    >
      ...
      {/* Blueprint corner readouts — mono coordinate-style annotations
          on small translucent capsules so they stay readable over the
          ambient video. */}
      {cornerReadouts.map((c, i) => (
        <span
          key={i}
          aria-hidden="true"
          className={`absolute ${c.pos} font-mono text-[10px] md:text-[11px] tracking-[0.24em] uppercase pointer-events-none px-2.5 py-1 rounded`}
          style={{
      ...
      {/* Full-bleed system rail — three-column control-room telemetry
          ...
          separated by thin green-on-green vertical dividers. Live
          status sits above on the left, build marker on the right. */}
      <div
        className="absolute left-0 right-0 bottom-0 z-10 pointer-events-none"
        style={{ background: "var(--process-primary)" }}
      >
        {/* Top meta row — live status + build marker, separated by a
            hairline rule that meets the column dividers below. */}
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16">
          ...
          >
            ...
            <div className="font-mono text-[10px] md:text-[11px] tracking-[0.24em] uppercase text-white/65">
              REV 02 / 2026
            </div>
          </div>
        </div>
        ...
        {/* Three-column capability grid — each column shows a large mono
            ...
            Vertical dividers run flush with the meta-row hairline above. */}
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16">
          <div className="grid grid-cols-3 text-white">
            {stripItems.map((item, i) => (
              ...
              >
                <span className="font-mono text-[18px] md:text-[22px] leading-none text-white/55 tabular-nums">
