...
export function HeroSection() {
  ...
  // that frame the editorial canvas without movement.
  const cornerReadouts = [
    { pos: "top-6 left-6 md:top-10 md:left-10", text: "N 12.97° / E 77.59°" },
    { pos: "top-6 right-6 md:top-10 md:right-10", text: "REV 02 / 2026" },
    {
      pos: "bottom-6 left-6 md:bottom-10 md:left-10",
      text: "INDEX 001 / HOME",
    },
    {
      pos: "bottom-6 right-6 md:bottom-10 md:right-10",
      text: "TVASTR · BLR",
    },
  ];
  ...
  return (
    ...
    >
      ...
      {/* Editorial wash — softens the video so the headline stays the
          centerpiece. A radial vignette of the canvas color brightens the
          headline area; a subtle green tint at the edges anchors the
          secondary brand color. */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse 75% 60% at 50% 45%, rgba(255,255,255,0.78) 0%, rgba(255,255,255,0.55) 45%, rgba(255,255,255,0.35) 100%)",
        }}
      />
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse 70% 50% at 50% 40%, rgba(0,60,51,0.06) 0%, transparent 70%)",
        }}
      />
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-6 md:px-12 lg:px-16 text-center">
        <p className="font-mono text-[11px] md:text-[12px] tracking-[0.28em] uppercase text-txt-muted mb-8 inline-flex items-center gap-2.5">
          <span
        ...
        {/* Hairline capability strip — three quiet product anchors. This
            replaces the rotating Forge_Core icosahedron as the
            centerpiece. The strip reads as Cohere's trust-anchor row
            without using customer logos we don't have. */}
        <div
          ref={stripRef}
          className="mt-16 md:mt-20 mx-auto max-w-3xl border-t border-border-default pt-8"
        >
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-px">
            {stripItems.map((item, i) => (
              <div
                key={i}
                className="text-left sm:px-6 py-1 sm:bg-white/60 backdrop-blur-[2px] rounded"
              >
