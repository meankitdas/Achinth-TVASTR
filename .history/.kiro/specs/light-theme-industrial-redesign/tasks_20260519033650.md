...
- [x] 2. Cohere_Kit adoption + token rewrite
  ...
  - [x] 2.2 Author the Cohere → Theme_Token mapping
  ...
  - [x] 2.3 Rewrite `src/design/colors.js` with Light_Theme hex values
  ...
  - [x] 2.4 Sync `src/index.css` `:root` variables and remove dark-only utilities
  ...
  - [x] 2.5 Confirm `tailwind.config.js` shape is unchanged
  ...
  - [x] 2.6 Refactor every hard-coded hex literal out of section components
...
- [x] 3. Animation infrastructure (GSAP + Framer Motion + reduced-motion)
  - [x] 3.1 Implement `src/animation/gsap.js` and register ScrollTrigger exactly once
  ...
  - [x] 3.2 Implement `src/animation/motion.js` (canonical presets) and a preset validator
  ...
  - [x] 3.3 Implement `src/hooks/useReducedMotion.js`
  ...
  - [x] 3.4 Implement `src/animation/MotionConfig.jsx` and broadcast reduced-motion via context
  ...
  - [x] 3.5 Replace `useScrollReveal` with `src/hooks/useSectionReveal.js`
  ...
  - [x] 3.6 Implement `src/hooks/useMagnetic.js`, `src/components/effects/CursorFollower.jsx`, `src/components/effects/PageTransition.jsx`, and wire them into `App.jsx`
...
- [x] 4. Industrial_Decor layer
  - [x] 4.1 Implement `src/design/clipPaths.js`
  ...
  - [x] 4.2 Implement `src/components/decor/BackgroundGrid.jsx`
  ...
  - [x] 4.3 Implement `src/components/decor/CornerGuides.jsx` and `src/components/decor/RulerTickDivider.jsx`
  ...
  - [x] 4.4 Mount the decor in `App.jsx` and apply machined-corner clips to every card surface
...
- [x] 5. Forge_Core adaptation
  - [x] 5.1 Adapt silhouette contrast and brand emissive in `src/three/ForgeCore.jsx`
  ...
  - [x] 5.2 Cap mobile pixel ratio and pause/resume the render loop with IntersectionObserver in `src/three/ForgeCore.jsx`
  ...
  - [x] 5.3 Implement WebGL context-loss SVG fallback and reduced-motion final pose in `src/three/ForgeCore.jsx`
  ...
  - [x] 5.4 Drive ambient idle pulse from the GSAP_Layer
...
- [x] 6. Hero choreography
  - [x] 6.1 Wire the labeled GSAP entry timeline inside `src/components/sections/HeroSection.jsx`
  ...
  - [x] 6.2 Attach scroll-linked Forge_Core parallax via ScrollTrigger and clean up on unmount
...
- [x] 7. Section reveals + home / marketing sections
  - [x] 7.1 Apply `useSectionReveal` to every section under `src/components/sections/**`
  ...
  - [x] 7.2 Retarget colors per Reqs 13.1–13.9 across all home / marketing sections
  ...
  - [x] 7.3 Insert `<RulerTickDivider />` between architectural zones in `DeploymentSection`
  ...
  - [x] 7.4 Retarget `ProductCard` defect grid and ProductSlider transition
  ...
  - [x] 7.5 Checkpoint — sections render light-themed and reveal cleanly
    - Ensure all tests pass, ask the user if questions arise.
    - _Requirements: 9.1, 9.6, 13.1, 13.10_
    - _Design: §Hero choreography; §Components and Interfaces > Industrial decor layer_

- [x] 8. System detail pages
  - [x] 8.1 Light-theme `RejectionAnalysisSystem.jsx` and `PlantIntelligence.jsx`
  ...
  - [x] 8.2 Drive `SystemImageBlock` reveal via `useSectionReveal`
  ...
  - [x] 8.3 Update `SystemWorkflow` connectors and `SystemImpactGrid` tiles
...
- [x] 9. Navbar + micro-interactions
  - [x] 9.1 Implement Navbar transparent ↔ surfaced state in `src/components/navigation/Navbar.jsx`
  ...
  - [x] 9.2 Add focus ring and active-link color in Navbar
    ...
    - _Design: §Components and Interfaces > Industrial decor layer (consumers table — Navbar)_
  - [x] 9.3 Apply `useMagnetic` to every primary CTA
  ...
  - [x] 9.4 Confirm `<CursorFollower />` mounting + coarse-pointer + reduced-motion guards
...
- [x] 10. Portal + Tier_Badge
  - [x] 10.1 Implement `tierToBadgeState(tier, productId)` in `src/lib/capabilities.js`
  ...
  - [x] 10.2 Implement `<Tier_Badge />` in `src/components/portal/TierBadge.jsx`
  ...
  - [x] 10.3 Wire `<Tier_Badge />` into `PortalDashboard`, `PortalDownloads`, `LockedProductCard`, `LockedScreen`, `UpgradeBanner`
  ...
  - [x] 10.4 Light-theme `PortalLogin`, `PortalDashboard`, `PortalDownloads`
  ...
  - [x] 10.5 Checkpoint — portal renders correct badges per tier
    - Ensure all tests pass, ask the user if questions arise.
    - _Requirements: 15.5, 15.6, 15.7, 15.8, 15.10_
    - _Design: §Tier-gating UX in the portal_

- [x] 11. Build & CI safeguards
  - [x] 11.1 Implement the `dark:` grep script
  ...
  - [x] 11.2 Implement the theme-integrity check
  ...
  - [x] 11.3 Implement the bundle-size diff script
  ...
  - [x] 11.4 Wire Lighthouse CI to the build pipeline
...
- [x] 13. Verification — build, performance, and end-to-end gating
  - [x] 13.1 Capture pre-feature bundle baselines
    ...
    - _Design: §Testing Strategy > Visual regression and build checks (Bundle-size diff)_
  - [x] 13.2 Run the full verify pipeline
  ...
  - [x] 13.3 Final checkpoint — full verification gate
    - Ensure all tests pass, ask the user if questions arise.
