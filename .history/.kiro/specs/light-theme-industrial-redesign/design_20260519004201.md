...
The full state machine driving the badge for a single product card, as a function of the user's license tier, is:

```mermaid
stateDiagram-v2
    [*] --> Resolving
    Resolving --> ACTIVE: tier grants productId directly
    Resolving --> INCLUDED: tier is full_stack and productId is ras_enterprise
    Resolving --> LOCKED: tier does not grant productId
    Resolving --> LOCKED: tier is unrecognized or undefined

    state ACTIVE {
        [*] --> Rendered
        Rendered: label = "ACTIVE"
        Rendered: fg = colors.signal.warning
        Rendered: border = colors.signal.warning, alpha 0.3-0.5
    }
    state INCLUDED {
        [*] --> Rendered
        Rendered: label = "INCLUDED"
        Rendered: fg = colors.text.muted
        Rendered: border = colors.text.muted, alpha 0.3-0.5
    }
    state LOCKED {
        [*] --> Rendered
        Rendered: fg = colors.text.muted
        Rendered: lock icon
        Rendered: upgrade CTA accent = colors.signal.warning
        Rendered --> UpgradeFlow: CTA activated
    }
    UpgradeFlow --> [*]
```
