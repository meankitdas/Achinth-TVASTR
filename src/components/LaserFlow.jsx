...
void mainImage(out vec4 fc,in vec2 frag){
    vec2 C=vec2(iResolution.x * 0.5, iResolution.y * uCenterYFrac); float invW=1.0/max(C.x,1.0);
    ...
    float cx=clamp(uvc_displaced.x/(R_H*uHLenFactor),-1.0,1.0),tH=clamp(TWO_PI-acos(cx),tauMin,tauMax);
    for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){
        float tu=tH+float(k)*DT_LOCAL,wt=tauWf(tu,tauMin,tauMax); if(wt<=0.0) continue;
        float spd=max(abs(sin(tu)),0.02),u=clamp((basePhase-tu)/max(uDecay,EPS),0.0,1.0),env=pow(1.0-abs(u*2.0-1.0),0.8);
        vec2 p=vec2((R_H*uHLenFactor)*cos(tu),0.0);
        a+=wt*bs(uvc_displaced,p,env*spd);
    }
    float yPix=uvc_displaced.y,cy=clamp(-yPix/(R_V*uVLenFactor),-1.0,1.0),tV=clamp(TWO_PI-acos(cy),tauMin,tauMax);
    for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){
        ...
        b+=wt*bsa(uvc_displaced,p,mask*env,sig);
    }
    ...
    float fog=0.0;
#if FOG_ON
    vec2 fuv=uvc_displaced*uFogScale;
    ...
    float radialFade = 1.0 - smoothstep(0.0, 0.7, length(uvc) / 250.0);
...
export const LaserFlow = ({
  ...
  useLayoutEffect(() => {
    ...
    // Force initial size calculation and render
    setSizeNow(true);
    renderer.render(scene, camera);
    
    // Ensure visibility with delayed renders
    setTimeout(() => {
      setSizeNow(true);
      renderer.render(scene, camera);
    }, 0);
    ...
    // Start with in-view = true, then update based on actual visibility
    ...
    setTimeout(() => io.observe(mount), 100);
    ...
    const animate = () => {
      ...
      const t = (performance.now() - startTimeForAnimation) / 1000;
      ...
      // Skip fade animation - start fully visible
      uniforms.uFade.value = 1;
      
      // Force render on first few frames to ensure visibility
