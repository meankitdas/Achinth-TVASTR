  ...
  return (
    <SectionShell id="energy-reasoning">
        ...
        {concepts.map((concept, i) => (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="text-sm font-bold text-txt-primary">{concept.name}</h4>
            </div>
            <p className="text-xs text-txt-secondary leading-relaxed">{concept.description}</p>
          </div>
        ))}
      </div>
      ...
      <div className="p-6 rounded-xl border border-border-default bg-bg-primary/60 mb-8">
        <h3 className="text-base font-bold text-txt-primary mb-4">Decision Tiers</h3>
        ...
        <div className="space-y-3">
          {decisionTiers.map((tier, i) => (
            <div key={i} className="flex items-start gap-3">
              <span className="text-xs font-mono text-txt-primary w-20 flex-shrink-0">{tier.threshold}</span>
              <span className="text-xs text-txt-secondary">{tier.description}</span>
            </div>
          ))}
        </div>
      </div>
