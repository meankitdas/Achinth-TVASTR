      ...
      <div className="space-y-4">
        {layers.map((layer, i) => (
          <div key={layer.id} className="relative">
            ...
            <div className="flex flex-col md:flex-row items-stretch gap-4 p-5 rounded-lg border border-metallic-800/20 bg-charcoal-950/50">
              {/* Layer label */}
              <div className="flex-shrink-0 md:w-48 flex items-center gap-3">
                ...
                <h4 className="text-sm font-bold text-metallic-100">{layer.name}</h4>
              </div>

              {/* Description */}
              <div className="flex-1">
                <p className="text-xs text-metallic-400 leading-relaxed mb-3">{layer.description}</p>
